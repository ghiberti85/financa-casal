...
27. **Modelos com *adaptive thinking* ligado por padrão (Sonnet 5, Opus 5) consomem `max_tokens` com o raciocínio, não só com a resposta visível** — `financial-assistant` usava `max_tokens: 500` e isso fazia toda pergunta real falhar (thinking consumia o orçamento inteiro antes de gerar texto, ou o texto ficava truncado). Além disso, `data.content` pode trazer um bloco `type:"thinking"` **antes** do bloco de texto — nunca indexar `content[0]` direto, sempre `content.find(b => b.type === "text")`. Fix aplicado (v3, 2026-09): `max_tokens: 1024` + `output_config: { effort: "low" }` (tarefa de narrativa simples não precisa de raciocínio profundo) + busca pelo bloco de texto. Ao integrar qualquer novo endpoint que chame Claude via Edge Function, sempre testar com uma pergunta real (não só em modo demo, que nem chega a chamar a API) antes de considerar pronto.
