...
23. **E-mail de recuperação de senha ignora `redirect_to` se a URL não estiver na allowlist** — `supabaseRecoverPassword()` envia `redirect_to=<window.location.origin>` para `/auth/v1/recover`, mas o GoTrue só honra isso se a URL estiver em Supabase Studio → Authentication → URL Configuration → Redirect URLs. Se não estiver, o link do e-mail cai silenciosamente no Site URL padrão do projeto (já observado em produção apontando pra `localhost:3000`) — sem erro visível, só o usuário clicando num link que não abre o app certo. Sempre conferir essa lista ao adicionar um novo domínio (preview do Vercel, produção, dev local).

24. **Cadastro (sign-up) escondido da UI intencionalmente, não removido** — este é um app pessoal para 2 usuários conhecidos (Fernando e esposa), não um produto público. `LoginPage` não expõe mais o link "Criar agora"/`setMode("signup")`, mas todo o código de cadastro (mode `signup`, steps `profile`/`family_setup`, `handleAuth` com branch signup) continua no arquivo — não remover sem aprovação explícita do usuário. Se precisar reabilitar no futuro (ex: convidar um terceiro usuário manualmente), basta trocar `setMode("login")` fixo por um novo entry point visual; a lógica já está pronta.

25. **`src/index.css` tem `#root { text-align: center }` global (sobra do template do Vite)** — qualquer texto novo (parágrafo, span dentro de flex row, div) herda `text-align:center` a menos que sobrescreva explicitamente com `textAlign:"left"`. Em telas curtas isso passa despercebido (texto de 1 linha centralizado não chama atenção), mas em texto de 2+ linhas quebra o layout visualmente (ex: `AIAssistantCard` — resposta da IA aparecia centralizada; `MonthlySummaryCard` — linha "maior gasto" com ícone + texto longo ficava com o ícone desalinhado porque o `<span>` de texto também centralizava e quebrava esquisito). **Sempre testar componente novo com texto propositalmente longo (2-3 linhas)**, não só com dado curto de exemplo — o bug só aparece com quebra de linha real. Ao criar linhas `display:flex` com ícone + texto que pode ser longo: usar `alignItems:"flex-start"` (não `"center"`) no container, `flexShrink:0` no ícone, e `minWidth:0,textAlign:"left"` no texto.

26. **`GoalsView.save()` não verifica `isDemo` antes de chamar `supabaseFetch`** — viola a armadilha #2 (sempre checar `isDemo` antes de qualquer operação no banco). Resultado: criar/editar meta no modo demo dispara um `HTTP 404` real em vez de atualizar só o estado local como os outros componentes demo fazem. Ainda não corrigido — identificado durante teste de responsividade (2026-09), registrado aqui para correção numa próxima sessão.

27. **Modelos com *adaptive thinking* ligado por padrão (Sonnet 5, Opus 5) consomem `max_tokens` com o raciocínio, não só com a resposta visível** — `financial-assistant` usava `max_tokens: 500` e isso podia fazer a pergunta falhar (thinking consumia o orçamento inteiro antes de gerar texto, ou o texto ficava truncado). Além disso, `data.content` pode trazer um bloco `type:"thinking"` **antes** do bloco de texto — nunca indexar `content[0]` direto, sempre `content.find(b => b.type === "text")`. Fix aplicado (v3, 2026-09): `max_tokens: 1024` + `output_config: { effort: "low" }` (tarefa de narrativa simples não precisa de raciocínio profundo) + busca pelo bloco de texto. Ao integrar qualquer novo endpoint que chame Claude via Edge Function, sempre testar com uma pergunta real (não só em modo demo, que nem chega a chamar a API) antes de considerar pronto.

28. **Antes de investigar lógica, ler o status HTTP real que a Anthropic devolveu** — o assistente falhava em 100% das perguntas e a causa raiz era simplesmente `401 authentication_error: API key is invalid` (chave do Vault expirada/revogada), não nada de código. O erro só ficou visível depois que a Edge Function passou a repassar a mensagem real da Anthropic e a logar com `console.error` (antes, tudo virava um "502 genérico"). Ordem de diagnóstico para qualquer falha do assistente: (1) ler o erro no card/toast — ele já traz `Anthropic <status>: <mensagem>`; (2) `query_logs` com `source = 'function_edge_logs'` pra ver os status codes; (3) `source = 'function_logs'` pros `console.error`. **401 = trocar a chave** em Supabase Studio → Project Settings → Vault → secret `ANTHROPIC_API_KEY` (a mesma chave é usada pela `analyze-import`, então a importação com IA quebra junto).
